        ...
        if(!getpeereid(client,&uid,&gid) && uid==console && uid!=0 && uid!=(uid_t)-1) @try {
            ...
            if([command isEqual:@"start"]) {
                ...
                if(!owlPID) {
                    ...
                    BOOL channelAvailable=NO;
                    for(CWChannel *channel in wifi.supportedWLANChannels)
                        if(channel.channelNumber==149) { channelAvailable=YES; break; }
                    AQRequire(channelAvailable,@"The Wi-Fi interface does not support AirDrop's configured channel 149.");
